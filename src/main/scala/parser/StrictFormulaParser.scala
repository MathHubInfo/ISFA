...
//
//class StrictFormulaParser extends JavaTokenParsers with PackratParsers {
//  var calls: BigInt = 0
//  var succeded: BigInt = 0
//  var exceptions: BigInt = 0
//
//  val successFile = new PrintWriter(new File("logs/strictSuc.txt"))
//  val failFile = new PrintWriter(new File("logs/strictFail.txt"))
//  val exceptionFile = new PrintWriter(new File("logs/strictException.txt"))
//
//
...
//
//  // avoid considering expressions of (-exp text ...) as a text
//  lazy val sen_cbracket: PackratParser[String] = ")" | "}" | "]" /*<~not("""[\^\+\*\-\/\\]""".r)*/
//  // avoid considering expr part of (expr)^exponent as seperate from exponent
//
...
//      sen_obracket ~ sentence ~ sen_cbracket ^? {
//        // ex: (fraction continues) //TODO: (-1) //try expr if fail try words
...
//        case x ~ del => Name(x) :: del.map(x => Delim(x))
//      } |
//      sen_obracket ~ words ~ sentence ~ sen_cbracket ^^ {
//        // ex: (fraction continues) //TODO: (-1) //try expr if fail try words
//        //        case ob~w~cb =>(Delim(ob) :: w) ::: (s :: Delim(cb) :: Nil)
