...
object FormulaParser extends JavaTokenParsers with PackratParsers {
  var calls : BigInt = 0
...
//      case (left, right) => Func("<=",ArgList(left :: right :: Nil))
  ...
  lazy val function: PackratParser[String] = "floor\\b".r | "ceiling\\b".r | "ceil\\b".r | "sqrt\\b".r |"log\\b".r | "sinh\\b".r |"sin\\b".r | "cosh\\b".r | "cos\\b".r |
    "tan\\b".r |"tg\\b".r | "ctg\\b".r | "deg\\b".r | "binomial\\b".r | "numerator\\b".r | "exp\\b".r | "phi\\b".r | "[A-Z]+[a-z_]*\\b(?!\\d)".r |
    "[a-z_]{3,}[a-zA-Z]*\\b(?=\\()".r /*| "[a-z_]{1}\\b(?!\\d)".r*/

  lazy val constant: PackratParser[String] = "Pi\\b".r | "pi\\b".r | "infinity\\b".r | "infty\\b".r
...
//    } |
  ...
  def main(args : Array[String]): Unit = {
    val test = "a(n+3) = sum{k = 0..floor(n/3), binomial(n-2k, k)(-1)^k*2^(n-3k)}"
    println("input : "+ test)
